	...
	public function Header() {
		if ($this->header_xobjid === false) {
			...
			*/
			...
			// header string
			$this->SetFont($headerfont[0], $headerfont[1], $headerfont[2]);
			$this->SetX($header_x+115); //MOVER A LA DERECHA
			$this->SetFillColor(200, 200, 200); //FONDO1
		//			$this->MultiCell($cw, $cell_height, $headerdata['string'], 0, '', 0, 1, '', '', true, 0, false, true, 0, 'T', false);
		$this->MultiCell($cw+90, $cell_height, $headerdata['string'], 0, 'C', 1, 1, '', '', true, 0, false, true, 0, 'T', false);
		  //$this->MultiCell($cw, $cell_height, $headerdata['string'], 0, 'C', 1, 1, '', '', true, 0, false, true, 0, 'T', false);
	...
	public function Header2() {
		if ($this->header_xobjid === false) {
			...
			*/
			// header string
			$this->SetFont($headerfont[0], $headerfont[1], $headerfont[2]);
			$this->SetX($header_x+80);

			// set color for background
			$this->SetFillColor(200, 200, 200);
			$cw = 85; //MARGEN DEL ENCABEZADO
			$this->MultiCell($cw, $cell_height, $headerdata['string'], 0, 'C', 1, 1, '', '', true, 0, false, true, 0, 'T', false);
	...
	*/
	...
	public function Footer2() {
		...
		*/
        ...
        // Page number
        $this->Cell(0, 10, $this->getAliasNumPage().'/'.$this->getAliasNbPages(), 0, false, 'R');

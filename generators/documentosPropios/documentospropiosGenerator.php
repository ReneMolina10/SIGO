    ...
    'btnOpciones' => array(
        ...
        'eliminar' => array(
         ...
         'ver_doc' => array(
            ...
            'href'  => "javascript:ver_doc_modal('[HASHDOC]')",
            'target'=> '',
            'class' => 'text-primary',
            'mostrar_si' => '"[HASHDOC]" != ""'
        ),
...
$codigoJS = <<<JS
// Asegura variable baseUrl disponible
var baseUrl = '${baseUrl}';  // $baseUrl viene de $baseUrl = BASE_URL;

// Función global
function ver_doc_modal(hashDoc){
    if(!hashDoc){ console.warn('HASHDOC vacío'); return; }

    if(!document.getElementById('modalVerDoc')){
        $('body').append(
            '<div class="modal fade" id="modalVerDoc" tabindex="-1">'+
              '<div class="modal-dialog modal-xl">'+
                '<div class="modal-content">'+
                  '<div class="modal-header">'+
                    ...
                    '</button>'+
                  '</div>'+
                  '<div class="modal-body p-0" id="contenedorDocModal" style="min-height:70vh;display:flex;align-items:center;justify-content:center;">'+
                    '<div class="text-center w-100 py-5">Cargando...</div>'+
                  '</div>'+
                '</div>'+
              '</div>'+
            '</div>'
        ...
        $('#contenedorDocModal').html('<div class="text-center w-100 py-5">Cargando...</div>');
    }

    $('#modalVerDoc').modal('show');

    var url = baseUrl + 'viewdocpropio/previsualizarpdf/' + hashDoc;
    // Debug
    // console.log('Cargando URL:', url);
    var iframe = '<iframe src="'+url+'" style="width:100%;height:70vh;border:0;"></iframe>';
    $('#contenedorDocModal').html(iframe);
}

// ... (tu código previo de selects) ...

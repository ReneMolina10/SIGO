...
{assign var="rutaBuscar"      value="{$BASE_URL}{$controlador}/buscar/{$parentId}"}
...
{assign var="mostrarTfoot"    value=$mostrarTfoot|default:true}
      ...
      <thead>
        <tr>
          {foreach from=$columnas item=c}
            <th>{$c.titulo|default:$c.campo}</th>
          {/foreach}
        </tr>
      </thead>
      <tbody></tbody>
      {if $mostrarTfoot|default:true}
      <tfoot>
        <tr>
            {foreach key=key item=c from=$columnas}
                ...
                {else}
                    <th id="thfoot_{$key}">{$c.titulo|default:$c.campo}</th>
                {/if}                    
            {/foreach}
        </tr>
      </tfoot>
  ...
  (function($){
    ...
    var config = {
      tableId:          '{$tableId|default:"grid"}',
      rutaBuscar:       '{$rutaBuscar|default:""}',
      name_crud_table:  '{$name_crud_table|default:""}',
      tablaResponsiva:  {$tablaResponsiva|default:true|json_encode},
      tablaScrollX:     {$tablaScrollX|default:false|json_encode},
      checkbox_column:  {$checkbox_column|default:false|json_encode},
      columnas:         {$columnas|json_encode},
      parentId:         '{$parentId|escape:'js'}',
      bPaginate:        {$bPaginate|default:true|json_encode},      
      bFilter:          {$bFilter|default:true|json_encode},
      bInfo:            {$bInfo|default:true|json_encode},
      mostrarTfoot:     {$mostrarTfoot|default:true|json_encode}, 
      root:            '{$_layoutParams.root}',
    ...
    var tableId          = '{$tableId|default:"tbl_default"}',
      ...
      /*initCrudTable({
        tableId:          '{$tableId}',
        rutaBuscar:       '{$rutaBuscar}',
        nameCrudTable:    '{$name_crud_table|default:""}',
        columnas:         {$columnas|json_encode},
        tablaResponsiva:  {$tablaResponsiva|default:true},
        tablaScrollX:     {$tablaScrollX|default:false},
        checkboxColumn:   {$checkbox_column|default:false}
      });*/
    ...
    /*function traer_conceptos(tableId = "", cfg = {}){
      ...
      _tables[tableId] = $tbl.DataTable({
        ...
        language:    { url: "{$_layoutParams.ruta_view}/plugins/datatables_1.10.21/language/Spanish.json" },
    ...
    traer_conceptos('{$tableId|escape:'js'}', config);

...
<div class="card">
  <div class="card-header">
    ...
    <div class="card-tools no-print"> 
      ...
      {if $bd.ocultarBtnAgregar|default:'false' == 'false' }

        {if isset($template.editForm) && $template.editForm == 'modal'}
          <a class="btn btn-sm btn-success" href="javascript:open_modal_to_edit()"><i class="fas fa-plus"></i> Registrar {$nomsingular}</a>
        {else}
          <a class="btn btn-sm btn-success" href="{$_layoutParams.root}{$controlador}/editar/0/0/0/{$filtro|default:""}"><i class="fas fa-plus"></i> Registrar {$nomsingular}</a>
        {/if} 
      {/if}
 ...
 {if $template.displayInfo.mode|default:'' == "sortlist"  }
  ...
  function traer_conceptos(){   
    ...
    var table = $('#grid').DataTable({
      ...
      "ajax": {
          'type': 'POST',
          "url": "{$_layoutParams.root}{$controlador}/buscar/{$filtro}",
          data: function ( d ) {
              //d.nivel = nivel;
          },
          beforeSend: function () {

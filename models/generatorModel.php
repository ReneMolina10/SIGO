...
class generatorModel extends Model
{
  ...
  */
  ...
  public function putPost($num, $datos, $form, $keyt, $infoTabla, $controlador)
  { //$cont,$post,$this->form,$key,$fila
    ...
    */
    ...
    foreach ($form as $key => $fila) {
      ...
      /*
      if (!empty($fila['encrypt'])) {
                  $fila["campo"] = " ENC( :".$fila["campo"]." ) ";
        ...
        if (isset($fila["campo"])) {
          ...
          if ($fila["tipo"] == "checkbox" or $fila["tipo"] == "select_multiple" or $fila["tipo"] == "dual_listbox") {
            ...
            if ($fila["tipo"] == "date") {
              ...
              if (!empty($fila['encrypt'])) {
                //$campo = "ENC( :".$fila["campo"]." )";
                $key = base64_encode(GENERATOR_TEXT_ENCRYPTION_KEY);
                $campo = "MY_ENCRYPT(:" . $fila["campo"] . ", '$key')";
    ...
    foreach ($this->form as $key => $fila) {
      if (isset($fila["campo"])) {
        ...
        if ($fila["tipo"] == "checkbox" or $fila["tipo"] == "select_multiple" or $fila["tipo"] == "dual_listbox") {
          ...
          if (!empty($fila['encrypt'])) {
            $key = base64_encode(GENERATOR_TEXT_ENCRYPTION_KEY);
            $campos = " MY_DECRYPT(" . $fila["campo"] . ",'$key') AS " . $fila["campo"] . "  , " . $campos;
